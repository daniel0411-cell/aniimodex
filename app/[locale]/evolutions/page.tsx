...
export default async function EvolutionsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const branchingFamilyEntries = evolutionFamilies.filter((family) => family.children.length > 1);
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <h2 className="text-xl font-bold text-text-primary">{t('branchingTitle')}</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-text-secondary">{t('branchingDescription', { count: branchingFamilyEntries.length })}</p>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {branchingFamilyEntries.map((family) => (
            <div key={family.name} className="border-l-4 border-accent bg-ink-card p-4">
              <h3 className="font-semibold text-text-primary">{family.name}</h3>
              <p className="mt-2 text-sm text-text-secondary">{family.children.map((child) => child.name).join(' / ')}</p>
            </div>
          ))}
        </div>
      </section>
